...
export class SimpleAudioEngine {
  ...
  private scheduleStep(pattern: LoopPattern, step: number, time: number) {
    this.playTone(pattern.lead[step], time, 'square', 0.12);
    this.playTone(pattern.harmony[step], time, 'square', 0.08);
    this.playTone(pattern.bass[step], time, 'triangle', 0.18);
    this.playNoise(pattern.noise[step], time, 0.05);
  }
  ...
  private playTone(event: StepEvent, time: number, type: OscillatorType, duration: number) {
    ...
    gain.gain.linearRampToValueAtTime(event.velocity, time + 0.01);
  ...
  private playNoise(event: StepEvent, time: number, duration: number) {
